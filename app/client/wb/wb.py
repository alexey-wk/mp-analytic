...
class WBClient:
    ...
    def get_finreports(self, report_date: datetime):
        params = self._create_finreport_params(report_date)
        res = self.client.get(finreports_url, cookies=self.cookies, params=params)
        res.raise_for_status()
        return res.json()
    ...
    def _create_finreport_params(self, report_date: datetime):
